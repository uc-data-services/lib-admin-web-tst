<?php
function skeleton_lhrd_form_alter(&$form, &$form_state, $form_id) {
 ...
 */
function skeleton_lhrd_preprocess_page(&$variables) {
  drupal_add_js('jQuery(document).ready(function($){$("#navigation .content > ul").mobileMenu({prependTo:"#navigation",combine:false,switchWidth:768,topOptionText:"Select page"});});jQuery.extend(Drupal.settings, { "themePath": "' . path_to_theme() . '" });',array('type' => 'inline', 'scope' => 'header'));
}
drupal_add_js(drupal_get_path('theme', 'skeleton_lhrd').'/js/accordion.js');
drupal_add_js(drupal_get_path('theme', 'skeleton_lhrd').'/js/carousel.js');
//EOF:JavaScript
